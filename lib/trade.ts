...
/** Optional resolution overrides. Kept as one object rather than trailing
 * positional args: `fresh` (live-fill preflight) and `strike`/`expiry` (the
 * strategy builder's per-leg pinning) were added independently and would
 * otherwise compete for the same argument slot. */
export type TradeQuoteOptions = {
  /** Bypass the orders cache — used right before a real fill so the signed
   * calldata is built against a book that hasn't gone stale. */
  fresh?: boolean;
  /** Resolve at this exact strike instead of nearest-ATM — used by the
   * strategy builder (lib/strategyQuote.ts) to price one leg of a multi-leg
   * structure at a strike already picked via getAvailableStrikes. */
  strike?: number;
...
export async function getTradeQuote(
  ...
  const { fresh = false, strike: strikeOverride, expiry: expiryOverride } = options;
  ...
  // Best listed maker order at the matched expiry: nearest the target strike
  // (ATM unless the caller pinned one), cheaper on ties.
