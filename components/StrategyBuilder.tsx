...
function responseData<T>(response: Response, data: T & { error?: string }) {
...
export function StrategyBuilder({ asset }: { asset: Asset }) {
  ...
  const [advice, setAdvice] = useState<AiStrategySuggestion | null>(null);
  ...
  useEffect(() => {
    if (!strategy || !validAmount) return;
    let cancelled = false;
    const load = async () => {
      setLoading(true);
      try {
        ...
        if (cancelled) return;
        setQuote(responseData(response, data));
        setQuoteError(null);
      } catch (error) {
        if (!cancelled) {
          setQuote(null);
  ...
  const chooseSentiment = (next: SentimentBucket) => {
  ...
  const askGonka = async () => {
    ...
    try {
      ...
      const result = responseData(response, await response.json()) as AiStrategySuggestion;
      setAdvice(result);
      setQuote(null);
      ...
      setAdvice(null);
      setAdviceError(error instanceof Error ? error.message : "strategy advisor failed");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4" aria-label="Multi-leg strategy planner">
      <div className="rounded-lg border border-blue/25 bg-bluesoft/30 p-3 text-[12px] leading-relaxed text-muted">
        Explore real Thetanuts strikes and long-option prices at one shared expiry. This is a strategy planner, not a bundle order: GammaShield never sends sequential multi-leg fills.
      </div>

      <div className="grid grid-cols-4 gap-1 rounded-lg bg-panel2 p-1">
        {SENTIMENTS.map((item) => (
          <button key={item.id} onClick={() => chooseSentiment(item.id)} aria-pressed={sentiment === item.id} className={`h-8 rounded-md text-[11px] font-semibold ${sentiment === item.id ? "bg-panel text-fg shadow-sm" : "text-muted hover:text-fg"}`}>
            {item.label}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-1">
        {strategies.map((item) => (
          <button key={item.id} onClick={() => { setQuote(null); setStrategyId(item.id); }} aria-pressed={strategy?.id === item.id} className={`flex items-center gap-3 rounded-lg border p-3 text-left ${strategy?.id === item.id ? "border-blue bg-bluesoft/30" : "border-edge hover:border-blue/40"}`}>
            <StrategyGlyph strategyId={item.id} />
            <span><span className="block text-[13px] font-semibold text-fg">{item.name}</span><span className="mt-0.5 block text-[11px] text-muted">{item.description}</span></span>
          </button>
        ))}
      </div>
      ...
      {validAmount && quoteError && <p className="text-[12px] text-crit">{quoteError}</p>}
      {activeQuote && (
        <div className="rounded-lg bg-panel2 p-3 text-[12px]">
          <div className="flex justify-between gap-3"><span className="text-muted">Shared expiry</span><span className="text-fg">{fmtExpiryDate(activeQuote.expiryTs)}</span></div>
          <div className="mt-2 flex flex-col gap-1.5">{activeQuote.legs.map((leg, index) => <div key={`${leg.side}:${leg.action}:${leg.strike}:${index}`} className="flex justify-between gap-3"><span className="text-muted">{leg.qty}× {leg.action} {leg.side} {fmtStrike(leg.strike)}</span><span className="text-fg">{leg.premiumPerContractUsd == null ? "Price unavailable" : `${fmtUsd(leg.premiumPerContractUsd, false)} / contract`}</span></div>)}</div>
          {activeQuote.netPremiumUsd != null ? <><div className="my-2 border-t border-edge" /><div className="flex justify-between font-semibold"><span>Long-option cost</span><span>{fmtUsd(activeQuote.netPremiumUsd, false, 2)}</span></div><StrategyPayoffChart legs={resolvedLegs} netPremiumPerUnit={activeQuote.netPremiumUsd / activeQuote.contracts} spot={activeQuote.spot} breakevens={activeQuote.breakevens} /><div className="grid grid-cols-2 gap-2 text-[11px]"><span className="text-muted">Max profit <b className="text-fg">{activeQuote.maxProfit === "unlimited" ? "Unlimited" : fmtUsd(activeQuote.maxProfit ?? 0, false)}</b></span><span className="text-muted">Max loss <b className="text-fg">{activeQuote.maxLoss === "unlimited" ? "Unlimited" : fmtUsd(activeQuote.maxLoss ?? 0, false)}</b></span></div></> : <p className="mt-3 rounded-md border border-warn/30 bg-panel p-2.5 text-muted">Thetanuts currently exposes reliable taker-long quotes only. This strategy includes a short leg, so its payoff is a planning template—not a price or executable order.</p>}
        </div>
      )}
